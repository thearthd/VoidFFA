...
import { gamesRef, claimGameSlot, releaseGameSlot, slotsRef, usersRef, requiredGameVersion, assignPlayerVersion, menuChatRef, authenticateToAllSlotApps} from './firebase-config.js';
        ...
        try {
            ...
            if (byUidSnap.exists()) {
                ...
                if (username && username.trim()) {
                    ...
                    const idSnap = await usersRef.child(key).child("ids").child(uid).once("value");
                    if (!idSnap.exists()) {
                        localStorage.removeItem("username");
                        username = "";
                    }
        ...
        btn.addEventListener("click", () => {
            ...
            if (wrapper) {
                ...
                if (typeof menuSong !== "undefined" && menuSong && typeof menuSong.pause === "function") {
                    try { menuSong.pause(); } catch (e) { /* ignore */ }
                }
