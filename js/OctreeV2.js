...
// =======================================================
// === DEMONSTRATION OF DATA INSPECTION AND DOWNLOAD ===
// =======================================================

// This part assumes you are in a browser environment where
// OctreeV2 is imported and used, for example, in a Three.js scene.

/**
 * Global variable to easily access the built Octree in the console.
 * You would assign your Octree instance to this.
 * Example: window.myBuiltOctree = octreeInstance;
 */
window.myBuiltOctree = null;

/**
 * Function to download Octree triangles as a JSON file.
 * Call this from your browser's console, e.g., `downloadOctreeTriangles()`.
 */
function downloadOctreeTriangles() {
    if (!window.myBuiltOctree || window.myBuiltOctree.triangles.length === 0) {
        console.warn("No Octree available or Octree has no triangles to download.");
        return;
    }

    console.log("Preparing Octree triangles for download...");

    try {
        // Map Three.js Triangle objects to a plain array of coordinates for JSON serialization
        const trianglesData = window.myBuiltOctree.triangles.map(t => ({
            a: t.a.toArray(),
            b: t.b.toArray(),
            c: t.c.toArray()
        }));

        const jsonString = JSON.stringify(trianglesData, null, 2); // Pretty-print JSON
        ...
        a.href = url;
        a.download = 'octree_collision_triangles.json'; // Suggested filename
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        console.log("Octree triangles download initiated successfully!");
    } catch (error) {
        console.error("Error downloading Octree triangles:", error);
        console.log("Possible causes: Circular references if you tried to stringify the full Octree object directly. Only serializing simple data like coordinates is recommended.");
    }
}

// Make the download function available globally for console use
window.downloadOctreeTriangles = downloadOctreeTriangles;


// Example of how you might use and inspect this in your main application logic:
/*
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import { Scene } from 'https://cdnjs.cloudflare.com/ajax/libs/three.js/0.152.0/three.module.js';

const loader = new GLTFLoader();
const scene = new Scene(); // Your actual scene
...
loader.load('path/to/your/model.gltf', (gltf) => {
    ...
    console.log("Starting Octree build from GLTF scene...");

    octree.fromGraphNode(gltf.scene, (progress) => {
        // This progress callback will give updates during the add and split phases
        console.log(`Octree Build Progress: ${(progress.loaded * 100).toFixed(2)}%`);
    }).then(() => {
        console.log("Octree build complete!");
        // Assign the built octree to the global variable for console inspection
        window.myBuiltOctree = octree;

        // --- Console Inspection Examples ---
        console.log('--- Octree Inspection Tips ---');
        console.log('You can now inspect the octree in your browser console:');
        console.log('  Type `myBuiltOctree` to explore the object.');
        console.log(`  Number of triangles in root: ${myBuiltOctree.triangles.length}`);
        console.log(`  Octree bounding box: ${myBuiltOctree.box.min.toArray()} to ${myBuiltOctree.box.max.toArray()}`);
        console.log(`  First 3 triangles (simplified):`);
        console.log(myBuiltOctree.triangles.slice(0, 3).map(t => ({ a: t.a.toArray(), b: t.b.toArray(), c: t.c.toArray() })));
        console.log('  To download all triangles, type `downloadOctreeTriangles()` in the console.');
        console.log('------------------------------');

    }).catch(error => {
        console.error("Error building Octree:", error);
    });
}, undefined, (error) => {
