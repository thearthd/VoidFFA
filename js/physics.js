...
        this.isGrounded     = false;
        this._currentGroundNormal = new THREE.Vector3(0, 1, 0); // Default to world up
        this._targetPlayerQuaternion = new THREE.Quaternion();
...
        this.camera.getWorldDirection(this.tempVector);
        // Project onto the player's current ground plane
        this.tempVector.sub(this._currentGroundNormal.clone().multiplyScalar(this.tempVector.dot(this._currentGroundNormal)));
...
        this.camera.getWorldDirection(this.tempVector);
        ...
        this.tempVector.sub(this._currentGroundNormal.clone().multiplyScalar(this.tempVector.dot(this._currentGroundNormal)));
        this.tempVector.normalize();
        this.tempVector.cross(this._currentGroundNormal); // Cross with current ground normal to get side vector
        return this.tempVector;
    }
...
        }
        ...
        // Project current player velocity onto the ground normal to separate vertical component
        const verticalVelocityOnGround = this.playerVelocity.dot(this._currentGroundNormal);
        const horizontalVelocity = this.playerVelocity.clone().sub(this._currentGroundNormal.clone().multiplyScalar(verticalVelocityOnGround));

        // Calculate the target horizontal velocity
        const targetVelocity = moveDirection.multiplyScalar(currentMoveSpeed);

        // Determine acceleration/deceleration rate based on grounded state and input
        let accelRate;

        if (this.isGrounded) {
            accelRate = moveDirection.lengthSq() > 0 ? PLAYER_ACCEL_GROUND : PLAYER_DECEL_GROUND;
        } else {
            accelRate = moveDirection.lengthSq() > 0 ? PLAYER_ACCEL_AIR : PLAYER_DECEL_AIR;
        }

        // Apply acceleration/deceleration to horizontal velocity components
        horizontalVelocity.lerp(targetVelocity, accelRate * deltaTime);
        this.playerVelocity.copy(horizontalVelocity);
        this.playerVelocity.addScaledVector(this._currentGroundNormal, verticalVelocityOnGround); // Re-add vertical component


        // Handle jumping
        if (this.isGrounded && input.jump) {
            this.playerVelocity.addScaledVector(this._currentGroundNormal, JUMP_VELOCITY); // Jump along the ground normal
...
     * This is called after all movement and collision resolution for a physics step.
     * @returns {THREE.Vector3 | null} The normal of the ground if grounded, otherwise null.
     */
    _checkIfGrounded() {
        if (!this.collider || !this.collider.geometry || !this.collider.geometry.boundsTree) {
            this.isGrounded = false;
...
            // Calculate the forward direction for the player, projected onto the ground normal's plane.
            // This ensures the player's "forward" is parallel to the ground.
            const playerWorldForward = new THREE.Vector3();
            this.camera.getWorldDirection(playerWorldForward);
            playerWorldForward.sub(this._currentGroundNormal.clone().multiplyScalar(playerWorldForward.dot(this._currentGroundNormal)));
...
            this._updatePlayerPhysics(FIXED_TIME_STEP);
            const groundNormal = this._checkIfGrounded(); // Get ground normal
            if (this.isGrounded && groundNormal) {
                this._currentGroundNormal.copy(groundNormal);
            } else if (!this.isGrounded) {
                this._currentGroundNormal.set(0, 1, 0); // Reset to world up if not grounded
            }
